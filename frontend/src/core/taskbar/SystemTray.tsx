...
import { Wifi, Volume2, Battery, Moon, Bell, ArrowLeft, Download, Upload, Activity, CheckCircle2, Info, AlertTriangle, CheckCircle, XCircle, Trash2 } from 'lucide-react';
...
export const SystemTray: React.FC<SystemTrayProps> = ({ onClose }) => {
    ...
    const mockSpeedTest = async (min: number, max: number, onProgress: (val: number) => void) => {
        let current = 0;
        ...
        try {
            ...
            try {
                ...
                if (response.ok && response.body) {
                    ...
                    finalDl = Number(((totalBytes * 8) / totalTime / (1024 * 1024)).toFixed(2));
                } else {
            ...
            try {
                ...
                if (uploadRes.ok) {
                    const totalTime = (performance.now() - start) / 1000;
                    finalUl = Number(((testData.length * 8) / totalTime / (1024 * 1024)).toFixed(2));
                } else {
                    throw new Error("Fallback required");
                }
            } catch (e) {
            ...
            {activePanel === 'main' ? (
                <div className="flex flex-col h-full p-4">
                    <div className="flex items-center justify-between mb-4 border-b border-slate-200/50 dark:border-slate-700/50 pb-2">
                        <span className="text-sm font-semibold truncate leading-none">Notifications & Stats</span>
                    ...
                    <div className="space-y-3 mb-4 flex-1 overflow-y-auto pr-1">
                        {notifications.length === 0 ? (
                            ...
                            notifications.map((notif) => {
                                ...
                                return (
                                    <div key={notif.id} className="group relative flex items-start space-x-3 p-3 bg-white/50 dark:bg-slate-800/80 rounded-lg shadow-sm border border-slate-100 dark:border-slate-700/50 transition-all hover:bg-white dark:hover:bg-slate-800">
                                        <div className={`p-2 rounded-full mt-1 ${colorClass}`}>
                                            <Icon className="w-4 h-4" />
                                        </div>
                                        <div className="flex-1 min-w-0 pr-6">
                                            <div className="flex items-center justify-between">
                                                <h4 className="text-sm font-semibold truncate pr-2">{notif.title}</h4>
                                                {notif.appName && (
                                            ...
                                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 leading-snug break-words pr-2">{notif.message}</p>
                                            <span className="text-[9px] text-slate-400 mt-2 block">{timeAgo}</span>
                                        </div>
                                        <button
                ...
                <div className="flex flex-col h-full animate-in slide-in-from-right-4 duration-300 p-4">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center py-2">
                        ...
                        <h3 className="font-semibold text-lg text-slate-800 dark:text-slate-100">Connected</h3>
                        <p className="text-xs text-slate-500 mb-6 font-medium bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full mt-2">OS_Secure_Network_5G</p>
                        ...
                        <div className="w-full bg-slate-50 dark:bg-slate-800/60 rounded-xl p-4 border border-slate-200/50 dark:border-slate-700/50">
                            <div className="flex justify-between items-center mb-4 text-sm font-medium">
                                <span className="flex items-center text-slate-600 dark:text-slate-300"><Activity className="w-4 h-4 mr-2 text-blue-500" /> Bandwidth Test</span>
                                {currentStatus && (
                                    <span className="text-[10px] text-blue-600 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/40 px-2 py-0.5 rounded-full animate-pulse">{currentStatus}</span>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 mb-4">
                                <div className="flex flex-col items-center justify-center bg-white dark:bg-slate-900 p-3 rounded-lg shadow-sm border border-slate-100 dark:border-slate-800">
                                    <div className="flex items-center text-slate-500 dark:text-slate-400 text-[10px] tracking-widest uppercase mb-1">
                                        <Download className="w-3 h-3 mr-1 text-emerald-500" /> Download
                                    </div>
                                    <span className="text-xl font-bold font-mono text-slate-800 dark:text-slate-100">
                                        {downloadSpeed !== null ? downloadSpeed.toFixed(1) : '--'}
                                        <span className="text-[10px] text-slate-500 ml-1 font-sans font-normal">Mbps</span>
                                    </span>
                                </div>
                                <div className="flex flex-col items-center justify-center bg-white dark:bg-slate-900 p-3 rounded-lg shadow-sm border border-slate-100 dark:border-slate-800">
                                    <div className="flex items-center text-slate-500 dark:text-slate-400 text-[10px] tracking-widest uppercase mb-1">
                            ...
                            >
                                ...
                                {!isTesting && uploadSpeed !== null && <CheckCircle2 className="w-4 h-4 ml-2 opacity-80" />}
                            </button>
                        </div>
                    </div>
                </div>
