...
export const ScorePanel: React.FC<ScorePanelProps> = ({ scores, mode, playerSymbol }) => {
    const xLabel = mode === 'single' ? (playerSymbol === 'X' ? 'You' : 'AI') : 'Player X';
    const oLabel = mode === 'single' ? (playerSymbol === 'O' ? 'You' : 'AI') : 'Player O';
...
const ScoreItem: React.FC<{ label: string; value: number; color: string }> = ({ label, value, color }) => (
