...
/* ═══════════════════════════════════════════════════
   Helpers
   ═══════════════════════════════════════════════════ */
function randomFood(cols: number, rows: number, snake: { x: number; y: number }[], now: number): FoodItem {
    ...
    if (rand > 0.92) {
        type = 'diamond'; pts = 50; ttl = 5000;
    } else if (rand > 0.78) {
        type = 'star'; pts = 25; ttl = 8000;
    }
...
/* ═══════════════════════════════════════════════════
   Component
   ═══════════════════════════════════════════════════ */
export const SnakeGame: React.FC<{ window: WindowInstance }> = ({ window: win }) => {
    ...
    const foodRef = useRef<FoodItem>({ x: 10, y: 10, type: 'apple', pts: 10, spawnedAt: 0, ttl: null });
    ...
    useEffect(() => {
        ...
        const speed = () => Math.max(MIN_SPEED, BASE_SPEED - (levelRef.current - 1) * SPEED_STEP);
        ...
        const tick = (now: number) => {
            ...
            particlesRef.current = particlesRef.current
                .map(p => ({ ...p, x: p.x + p.vx, y: p.y + p.vy, life: p.life + 1, vy: p.vy + 0.08 }))
            ...
            if (now - lastTickRef.current >= speed()) {
                ...
                if (head.x === food.x && head.y === food.y) {
                    ...
                    // Particles
                    const pColor = food.type === 'diamond' ? '#00c8ff' : food.type === 'star' ? '#ffd700' : '#ff4444';
                    particlesRef.current.push(...spawnParticles(
                        food.x * CELL + CELL / 2, food.y * CELL + CELL / 2, pColor
                    ));
    ...
    /* ─── Render function ─── */
    const render = useCallback((ctx: CanvasRenderingContext2D, w: number, h: number, now: number) => {
        const { cols, rows } = gridRef.current;
        ...
        if (food.ttl !== null) {
            ...
            if (remaining < 2000 && Math.floor(now / 100) % 2 === 0) drawFood = false;
        }

        if (drawFood) {
            const foodColors: Record<FoodItem['type'], { fill: string; glow: string }> = {
            ...
            if (food.type === 'diamond') {
                ...
                for (let i = 0; i < 5; i++) {
                    ...
                    const innerR = CELL / 4;
                    if (i === 0) ctx.moveTo(fx + Math.cos(outerAngle) * outerR, fy + Math.sin(outerAngle) * outerR);
                    else ctx.lineTo(fx + Math.cos(outerAngle) * outerR, fy + Math.sin(outerAngle) * outerR);
                    ctx.lineTo(fx + Math.cos(innerAngle) * innerR, fy + Math.sin(innerAngle) * innerR);
                }
                ctx.closePath();
            } else {
                // Circle apple
        ...
        snake.forEach((seg, i) => {
            ...
            ctx.quadraticCurveTo(sx + CELL - 1, sy + CELL - 1, sx + CELL - 1 - radius, sy + CELL - 1);
            ...
            if (i === 0) {
                ...
                ctx.beginPath();
                ctx.arc(cx - 3 + eyeOffX * 1.2, cy - 3 + eyeOffY * 1.2, 1.2, 0, Math.PI * 2);
                ctx.arc(cx + 3 + eyeOffX * 1.2, cy - 3 + eyeOffY * 1.2, 1.2, 0, Math.PI * 2);
                ctx.fill();
            }
        });
    ...
    const startGame = useCallback(() => {
        ...
        foodRef.current = randomFood(cols, rows, snakeRef.current, performance.now());
        ...
        const handleKey = (e: KeyboardEvent) => {
            ...
            switch (e.key) {
                case 'ArrowUp': case 'w': case 'W':
                    if (cur.dy !== 1) nextDirRef.current = { dx: 0, dy: -1 }; break;
                case 'ArrowDown': case 's': case 'S':
                    if (cur.dy !== -1) nextDirRef.current = { dx: 0, dy: 1 }; break;
                case 'ArrowLeft': case 'a': case 'A':
                    if (cur.dx !== 1) nextDirRef.current = { dx: -1, dy: 0 }; break;
    ...
    useEffect(() => {
        ...
        const draw = (now: number) => {
            ...
            for (let i = 0; i < 8; i++) {
                ...
                ctx.quadraticCurveTo(sx + CELL - 1, sy + CELL - 1, sx + CELL - 1 - r, sy + CELL - 1);
    ...
    const currentSpeed = Math.max(MIN_SPEED, BASE_SPEED - (level - 1) * SPEED_STEP);
    const speedPct = Math.round(((BASE_SPEED - currentSpeed) / (BASE_SPEED - MIN_SPEED)) * 100);
    ...
    return (
        <div className="snake-game-root">
                ...
                <div className="snake-hud-right">
                    <div className="snake-hud-stat">
                        ...
                        <div className="snake-speed-bar">
                            <div className="snake-speed-fill" style={{ width: `${speedPct}%` }} />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="snake-canvas-area" ref={containerRef}>
                    ...
                    <div className="snake-overlay">
                        ...
                        <div className="snake-stats-grid">
                            <div className="snake-stat-item">
                                ...
                                <span className={`value ${newRecord ? 'new-record' : ''}`}>{finalStats.score}</span>
                            </div>
                            <div className="snake-stat-item">
                            ...
                            {newRecord && (
                                <div className="snake-stat-item">
                                    ...
                                    <span className="value new-record" style={{ fontSize: 14 }}>🏆 NEW RECORD!</span>
                                </div>
                            )}
                        </div>
                        <button className="snake-play-btn" onClick={startGame}>
                ...
                {/* ─── Pause overlay ─── */}
                {isPaused && (
                    <div className="snake-overlay" style={{ background: 'rgba(10,14,39,0.7)' }}>
                        <div className="snake-title" style={{ fontSize: 'clamp(20px, 3vw, 36px)' }}>PAUSED</div>
                ...
                <div className="snake-mobile-controls">
                    <button className="snake-dpad-btn up" onPointerDown={() => handleDpad(0, -1)}>▲</button>
                    <button className="snake-dpad-btn left" onPointerDown={() => handleDpad(-1, 0)}>◄</button>
                    <button className="snake-dpad-btn right" onPointerDown={() => handleDpad(1, 0)}>►</button>
                    <button className="snake-dpad-btn down" onPointerDown={() => handleDpad(0, 1)}>▼</button>
                </div>
            </div>
        </div>
    );
