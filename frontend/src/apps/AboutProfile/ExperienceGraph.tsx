...
function buildPath(data: { year: number; value: number }[], xScale: (y: number) => number, yScale: (v: number) => number): string {
    return data.map((d, i) => `${i === 0 ? 'M' : 'L'}${xScale(d.year)},${yScale(d.value)}`).join(' ');
}
...
export const ExperienceGraph: React.FC = () => {
    ...
    const xScale = (year: number) => PADDING.left + ((year - minYear) / (maxYear - minYear)) * INNER_W;
    const yScale = (value: number) => PADDING.top + INNER_H - (value / 100) * INNER_H;
    ...
    return (
        <div>
            ...
            <div className="ap-graph-container">
                <svg className="ap-graph-svg" viewBox={`0 0 ${CHART_W} ${CHART_H}`} preserveAspectRatio="xMidYMid meet">
                    {/* Grid lines */}
                    {yTicks.map(t => (
                        <line
                            key={t}
