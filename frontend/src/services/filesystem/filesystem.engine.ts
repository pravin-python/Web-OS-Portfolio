...
function flattenStaticTree(
    ...
    const nodeIsSystem = isSystem || (parentId === 'root' && SYSTEM_DIRS.has(node.name));
...
function getParentAndName(pathStr: string): { parentPath: string; name: string } | null {
    const parts = pathStr.split('/').filter(Boolean);
...
export function mkdir(currentPath: string, name: string): FsResult {
    ...
    if (isSystemPath(currentPath)) {
        return { ok: false, error: 'Permission denied: cannot modify system directories' };
    }
    ...
    if (!parent) return { ok: false, error: `Directory not found: ${parsed.parentPath}` };
    if (parent.type !== 'folder') return { ok: false, error: `Not a directory: ${parsed.parentPath}` };

    // Check for duplicate
    const existing = findByPath(fullPath);
    if (existing) return { ok: false, error: `File or directory already exists: ${parsed.name}` };
...
export function touch(currentPath: string, name: string): FsResult {
    ...
    // Check for duplicate
    const existing = findByPath(fullPath);
    if (existing) return { ok: false, error: `File already exists: ${parsed.name}` };
 ...
 */
export function writeFile(currentPath: string, fileName: string, content: string): FsResult {
    const fullPath = resolvePath(currentPath, fileName);

    if (isSystemPath(fullPath)) {
        return { ok: false, error: 'Permission denied: cannot modify system files' };
    }
...
export function removeNode(currentPath: string, name: string): FsResult {
    ...
    if (fullPath === '/') {
        return { ok: false, error: 'Permission denied: cannot delete root directory' };
    }
    ...
    if (node.isSystem) {
        return { ok: false, error: 'Permission denied: cannot delete system files' };
    }
 ...
 */
export function moveNode(currentPath: string, srcName: string, destName: string): FsResult {
    ...
    if (!srcNode) return { ok: false, error: `No such file or directory: ${srcName}` };
    if (srcNode.isSystem) return { ok: false, error: 'Permission denied: cannot move system files' };
    ...
    if (destNode && destNode.type === 'folder') {
        ...
        if (childIds) {
            for (const cid of childIds) {
                ...
                if (child && child.name === srcNode.name) {
                    return { ok: false, error: `File already exists in destination: ${srcNode.name}` };
                }
            }
        }
        // Move to new parent
        ...
        persist();
        return { ok: true, message: `Moved ${srcName} → ${getPathString(destNode.id)}/${srcNode.name}` };
    }
    ...
    if (!destParent) return { ok: false, error: `Destination directory not found: ${parsed.parentPath}` };

    // Check collision
    const collision = findByPath(destPath);
    if (collision) return { ok: false, error: `File already exists: ${parsed.name}` };
 ...
 */
export function copyNode(currentPath: string, srcName: string, destName: string): FsResult {
    const srcPath = resolvePath(currentPath, srcName);
    const destPath = resolvePath(currentPath, destName);

    const srcNode = findByPath(srcPath);
    if (!srcNode) return { ok: false, error: `No such file: ${srcName}` };
    if (srcNode.type !== 'file') return { ok: false, error: 'cp: copying directories is not supported' };
    ...
    const destParent = findByPath(parsed.parentPath);
    if (!destParent) return { ok: false, error: `Destination directory not found: ${parsed.parentPath}` };
    if (destParent.isSystem) return { ok: false, error: 'Permission denied: cannot copy into system directories' };
