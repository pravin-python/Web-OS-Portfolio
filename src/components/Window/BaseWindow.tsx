...
export const BaseWindow: React.FC<BaseWindowProps> = ({ window, children }) => {
  ...
  const maxHeight =
    (isTouch ? VIRTUAL_HEIGHT : viewportSize.height) - (isTouch ? 56 : 48);
    ...
    >
      ...
      >
        ...
        <div
          className={twMerge(
            "flex items-center",
            isTouch ? "space-x-2" : "space-x-1",
          )}
        >
          <button
            onClick={() => minimizeWindow(window.id)}
            className={twMerge(
              "hover:bg-white/20 rounded-md transition-colors",
              isTouch ? "p-2" : "p-1",
            )}
          >
          ...
          >
            {window.isMaximized ? (
              <Copy size={isTouch ? 18 : 14} />
            ) : (
              <Square size={isTouch ? 18 : 14} />
            )}
          </button>
          <button
            onClick={handleClose}
            className={twMerge(
              "hover:bg-red-500 hover:text-white rounded-md transition-colors",
              isTouch ? "p-2" : "p-1",
            )}
          >
