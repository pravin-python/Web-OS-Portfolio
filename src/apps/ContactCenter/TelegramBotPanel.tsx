...
export const TelegramBotPanel: React.FC = () => {
  ...
  const cooldownRef = useRef(false); // Used only to prevent duplicate requests outside of render
  ...
  const handleSend = async () => {
    ...
    if (!CONTACT.botToken || !CONTACT.telegram.chatId) {
      setStatus("error");
      setErrorMsg(
        "Bot is not configured. Please set VITE_TELEGRAM_BOT_TOKEN and VITE_TELEGRAM_CHAT_ID.",
      );
      return;
    }
    ...
    try {
      const res = await fetch(
        `https://api.telegram.org/bot${CONTACT.botToken}/sendMessage`,
        {
          method: "POST",
      ...
      if (!res.ok) {
        ...
        throw new Error(
          data.description || `Telegram API error (${res.status})`,
        );
      }
    ...
    <div className="flex flex-col h-full p-6 space-y-4 overflow-y-auto">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Message delivered directly to my Telegram
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3 flex-1">
        ...
        <div>
          <label className="text-xs font-medium text-slate-600 dark:text-slate-400 mb-1 block">
            Message *{" "}
            <span className="text-slate-400 float-right">
              {message.length}/{MAX_MSG_LENGTH}
            </span>
          </label>
          <textarea
      ...
      <button
        ...
        className={`w-full py-2.5 rounded-lg text-sm font-semibold transition-all flex items-center justify-center shadow-sm ${
          !isValid || status === "sending" || isCooldown
