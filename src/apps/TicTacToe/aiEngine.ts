...
function computeMove(board: Board, ai: Player, d: Difficulty): number {
  switch (d) {
    ...
    case "medium":
      return mediumMove(board, ai);
    case "hard":
      return hardMove(board, ai);
    case "expert":
      return expertMove(board, ai);
  }
}
...
function expertMove(board: Board, ai: Player): number {
  ...
  for (const m of getAvailableMoves(board)) {
    const b = makeMove(board, m, ai);
    const score =
      minimaxAB(b, false, ai, -Infinity, Infinity) + positionBonus(m);
    if (score > bestScore) {
      bestScore = score;
      bestMove = m;
    }
...
function minimax(board: Board, isMaximizing: boolean, ai: Player): number {
  ...
  if (isMaximizing) {
    ...
    for (const m of moves) {
      best = Math.min(
        best,
        minimax(makeMove(board, m, opponent(ai)), true, ai),
      );
    }
    return best;
  }
...
function minimaxAB(
  ...
  if (isMaximizing) {
    ...
    for (const m of moves) {
      best = Math.max(
        best,
        minimaxAB(makeMove(board, m, ai), false, ai, alpha, beta),
      );
    ...
    for (const m of moves) {
      best = Math.min(
        best,
        minimaxAB(makeMove(board, m, opponent(ai)), true, ai, alpha, beta),
      );
