...
import {
    createBoard, makeMove as engineMove, checkWinner, isDraw, opponent,
} from './gameEngine';
...
export function useTicTacToe() {
    ...
    const finishGame = useCallback((result: WinResult | null) => {
        if (result) {
            setWinResult(result);
            setGameStatus('won');
            ...
            if (mode === 'single') {
                ...
                addGlobalNotification(
                    ...
                    won ? 'You beat the AI opponent! Great algorithmic thinking.' : 'You lost to the AI opponent.',
                    won ? 'success' : 'warning',
                    'TicTacToe'
                );
            } else {
    ...
    const doAITurn = useCallback(async (currentBoard: Board, aiPlayer: Player) => {
    ...
    const humanMove = useCallback((index: number) => {
    ...
    }, [board, currentPlayer, gameStatus, aiThinking, mode, playerSymbol, finishGame, doAITurn]);
    ...
    const setMode = useCallback((m: GameMode) => {
        ...
        aiLock.current = false;
        setAiThinking(false);
        notify(m === 'single' ? 'Single Player Mode 🤖' : 'Double Player Mode 👥', 'info');
    }, [notify]);

    const setDifficulty = useCallback((d: Difficulty) => {
        setDiffState(d);
        notify(`Difficulty: ${d.charAt(0).toUpperCase() + d.slice(1)} ⚙️`, 'info');
    }, [notify]);

    const setPlayerSymbol = useCallback((s: Player) => {
        setPlayerSym(s);
    ...
    return {
        board, currentPlayer, mode, difficulty, playerSymbol,
        scores, winResult, gameStatus, aiThinking, notifications,
        humanMove, newGame, setMode, setDifficulty, setPlayerSymbol, resetScore,
    };
}
