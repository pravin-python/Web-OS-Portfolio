...
export const Terminal: React.FC = () => {
  ...
  >([
    {
      type: "output",
      text: 'Research Station Terminal v2.0 — Type "help" for commands, "neofetch" for system info.',
    },
  ]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === "Enter" && inputVal.trim()) {
      ...
      setHistory((prev) => [
        ...prev,
        { type: "command", text: `${currentPath}$ ${cmd}` },
      ]);
      ...
      if (result.output.startsWith("__OPEN__")) {
        ...
        if (appKey) {
          ...
          setHistory((prev) => [
            ...prev,
            { type: "output", text: `Launching ${appName}...` },
          ]);
        } else {
          setHistory((prev) => [
            ...prev,
            { type: "error", text: `Unknown application: ${appName}` },
          ]);
        }
        return;
      }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center">
        <span className="mr-2 text-cyan-400 whitespace-nowrap flex-shrink-0">
          {currentPath}$
        </span>
        <input
          ref={inputRef}
