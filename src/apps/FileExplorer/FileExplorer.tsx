    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 border border-transparent">
      ...
      {!selectedFile && (
        <div className="flex-1 p-4 overflow-y-auto w-full grid grid-cols-[repeat(auto-fill,minmax(120px,1fr))] content-start gap-4">
          {nodes.length === 0 ? (
            <div className="w-full text-center py-10 opacity-50 text-sm">
              Empty folder
            </div>
          ) : (
            nodes.map((node) => (
              ...
              >
                {node.type === "folder" ? (
                  ...
                  <File
                    size={40}
                    className="text-blue-400 group-hover:scale-105 transition-transform"
                  />
                )}
                <span className="mt-2 text-xs truncate w-full px-1 text-slate-700 dark:text-slate-300">
                  {node.name}
                </span>
              </div>
            ))
