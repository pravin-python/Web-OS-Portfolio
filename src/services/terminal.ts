...
export function executeCommand(
  ...
  if (BLOCKED_COMMANDS.has(cmd)) {
    return {
      output: `Permission denied: '${cmd}' is not available in this environment.`,
    };
  }
  ...
  switch (cmd) {
    ...
    case "run":
      return {
        output:
          args.length > 0 ? `__OPEN__${args[0]}` : "Usage: open <app_name>",
      };
    case "neofetch":
      return cmdNeofetch();
    default:
...
function cmdHash(args: string[]): TerminalOutput {
  ...
  const pseudo = (hex + hex.split("").reverse().join("") + hex + hex).slice(
    0,
    32,
  );
