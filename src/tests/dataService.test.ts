...
import { searchData, CSVData } from "../services/dataService";
...
describe("searchData", () => {
  ...
  it("should return the correct matching row(s) (case insensitive)", () => {
    ...
    expect(result.rows[0]).toEqual(["1", "Alice Smith", "Engineer", "New York"]);
    expect(result.totalRows).toBe(1);

    const result2 = searchData(mockData, "SMITH");
    expect(result2.rows).toHaveLength(1);
    expect(result2.rows[0]).toEqual(["1", "Alice Smith", "Engineer", "New York"]);

    const result3 = searchData(mockData, "engineer");
    expect(result3.rows).toHaveLength(2);
    expect(result3.rows[0]).toEqual(["1", "Alice Smith", "Engineer", "New York"]);
    expect(result3.rows[1]).toEqual(["3", "Charlie Brown", "Engineer", "London"]);
  ...
  it("should search across all columns", () => {
    ...
    expect(resultId.rows[0]).toEqual(["2", "Bob Johnson", "Manager", "San Francisco"]);

    // Search in City
    const resultCity = searchData(mockData, "New York");
    expect(resultCity.rows).toHaveLength(2);
    expect(resultCity.rows[0]).toEqual(["1", "Alice Smith", "Engineer", "New York"]);
    expect(resultCity.rows[1]).toEqual(["4", "Diana Prince", "Director", "New York"]);
  });
